...
def pivoteamento_parcial(A,b,k):
    ...
    el_original = [el for el in b[k]]
    
    b[k] = b[vencedor]
    
    b[vencedor] = el_original

def pivoteamento_total(A,b,k):
    pass

def escalonador_de_sistemas(A,b,metodo=pivoteamento_parcial,imprimir=False):
    ...
    for k in range(n):
        ...
        for i in range(k+1,n):
            ...
            # processando cada elemento aᵢⱼ
            for j in range(n):
                A[i][j] -= multiplicador * A[k][j]
                
            # processando os elementos bᵢ do vetor resposta
...
def produz_c_e_g (A,b,imprimir=False,debug=False):
    ...
    while zero_na_diagonal:    
        for i in range(n):
            if A[i][i] == 0:
                ...
                if imprimir: 
                    print(f"Elemento {i+1} da diagonal é zero")
                    ut.imprime_matriz(A)
                # movendo a linha
                
                if debug:
                    print(f"Sistema antes de mexer no passo {passos}")
                    ut.imprime_matriz(A)
                    ut.imprime_matriz(b)
                temp = [ el for el in A[i]]
                A[i] = A[i-1]
                A[i-1] = temp
                temp = [ el for el in b[i]]
                b[i] = b[i-1]
                b[i-1] = temp
                if debug:
                    print(f"Sistema depois de mexer no passo {passos}\n")
                    ut.imprime_matriz(A)
                    ut.imprime_matriz(b)
                passos += 1
                break
    ...
    teste = criterio_das_linhas(A,imprimir=imprimir)
    if teste == False:
        if input("Critério das linhas não cumprido. Continuar? (s/n) ") == "n": raise Exception("Operação cancelada pelo usuário")
            
    if imprimir:
        print("Matrizes prontas para fazer C e g:")
        ut.imprime_matriz(A)
        ut.imprime_matriz(b)
    ###########################
    ...
    if imprimir: 
        print("C e g produzidos:")
        ut.imprime_matriz(C)
        ut.imprime_matriz(g)
        print("\n## Função produz_c_e_g terminada ##\n")
        
    return ut.array(C),ut.array(g)
